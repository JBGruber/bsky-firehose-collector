...
export default function (server: Server, ctx: AppContext) {
  server.app.bsky.feed.getFeedSkeleton(async ({ params, req }) => {
    ...
    let requesterDid = process.env.COLLECTOR_PUBLISHER_DID || 'did:plc:toz4no26o2x4vsbum7cp4bxp';
    try {
    ...
    const skipWhitelistCheck = process.env.COLLECTOR_SUBSCRIBER_ONLY === 'false';
